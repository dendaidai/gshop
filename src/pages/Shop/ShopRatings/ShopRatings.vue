...
export default {
  ...
  watch:{
    ...
    ratings(value){
      this.$nextTick(()=>{
        const bs = new BScroll(this.$refs.ratings,{
          click: true
        })
      })
    },
  ...
  computed:{
    ...
    filterRatings(){
      ...
      //根据selectType和onlyShowText过滤数组
      const filterArr = this.ratings.filter((rating)=>{
        //条件1（selectType）
        //当selectType===2时，返回true;当selectType===rating.rateType时，返回true
        //selectType===2 || selectType===rating.rateType

        //条件2(onlyShowText)
        //当onlyShowText===true,返回有rating.text的评论 rating.text
        //当onlyShowText===false,返回所有这个类型的评论 !onlyShowText
        //!onlyShowText || rating.text

        return (selectType===2 || selectType===rating.rateType) && (!onlyShowText || rating.text)
      })
      return filterArr
    },
    ...
    //点赞评论的个数
    positiveSize(){
      return this.ratings.reduce((size,item)=>{
        return item.rateType===0 ? size+1 : size
      },0)
    },
  },
  components:{
